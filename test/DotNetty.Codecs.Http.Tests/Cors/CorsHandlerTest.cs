...
namespace DotNetty.Codecs.Http.Tests.Cors
{
    ...
    public sealed class CorsHandlerTest
    {
        ...
        {
            var origin = new AsciiString("null");
            CorsConfig config = CorsConfigBuilder.ForOrigin(origin).WithAllowCredentials().Build();
            IHttpResponse response = PreflightRequest(config, origin.ToString(), "content-type, xheader1");
            Assert.Equal("true", response.Headers.Get(HttpHeaderNames.AccessControlAllowCredentials));
        }
        ...
        public void SimpleRequestCustomHeaders()
        {
            CorsConfig config = CorsConfigBuilder.ForAnyOrigin()
                .WithExposeHeaders((AsciiString)"custom1", (AsciiString)"custom2").Build();
        ...
        {
            CorsConfig config = CorsConfigBuilder.ForAnyOrigin().WithAllowCredentials().Build();
            IHttpResponse response = SimpleRequest(config, "http://localhost:7777");
            Assert.Equal("true", response.Headers.Get(HttpHeaderNames.AccessControlAllowCredentials));
        }
        ...
        {
            CorsConfig config = CorsConfigBuilder.ForAnyOrigin()
                .WithExposeHeaders((AsciiString)"one", (AsciiString)"two").Build();
            IHttpResponse response = SimpleRequest(config, "http://localhost:7777");
        ...
        {
            CorsConfig config = CorsConfigBuilder.ForOrigin((AsciiString)"https://localhost").WithShortCircuit().Build();
        ...
        static IHttpResponse SimpleRequest(CorsConfig config, string origin, string requestHeaders, HttpMethod method)
        {
            ...
            if (origin != null)
            {
                httpRequest.Headers.Set(HttpHeaderNames.Origin, (AsciiString)origin);
            }
            if (requestHeaders != null)
            {
                httpRequest.Headers.Set(HttpHeaderNames.AccessControlRequestHeaders, (AsciiString)requestHeaders);
            }
        ...
        static IFullHttpRequest CreateHttpRequest(HttpMethod method) => 
            new DefaultFullHttpRequest(HttpVersion.Http11, method, "/info");
        ...
        {
            ICharSequence header = response.Headers.Get((AsciiString)headerName);
