...
namespace DotNetty.Codecs.Http.Tests
{
    ...
    public sealed class QueryStringEncoderTest
    {
        ...
        public void DefaultEncoding()
        {
            ...
            e.AddParam("a", "b=c");
            Assert.Equal("/foo?a=b%3Dc", e.ToUriString());

            e = new QueryStringEncoder("/foo/\u00A5");
            e.AddParam("a", "\u00A5");
            Assert.Equal("/foo/\u00A5?a=%C2%A5", e.ToUriString());

            e = new QueryStringEncoder("/foo");
            e.AddParam("a", "1");
            e.AddParam("b", "2");
            Assert.Equal("/foo?a=1&b=2", e.ToUriString());
            ...
            e.AddParam("c", null);
            e.AddParam("d", null);
            Assert.Equal("/foo?a=1&b=&c&d", e.ToUriString());
        }
        ...
        public void NonDefaultEncoding()
        {
            ...
            Assert.Equal("/foo/\u00A5?a=%00%A5", e.ToUriString());
        }
        ...
        public void WhitespaceEncoding()
        {
            var e = new QueryStringEncoder("/foo");
            e.AddParam("a", "b c");
            Assert.Equal("/foo?a=b%20c", e.ToUriString());
        }
    }
}
