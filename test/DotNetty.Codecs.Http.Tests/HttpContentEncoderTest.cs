...
namespace DotNetty.Codecs.Http.Tests
{
    ...
    public sealed class HttpContentEncoderTest
    {
        ...
        public void ChunkedContentWithTrailingHeader()
        {
            ...
            content.TrailingHeaders.Set((StringCharSequence)"X-Test", (AsciiString)"Netty");
        ...
        public void FullContent()
        {
            ...
            res.Headers.Set(HttpHeaderNames.ContentLength, 42);
            ch.WriteOutbound(res);
        ...
        public void EmptyFullContentWithTrailer()
        {
            ...
            Assert.Equal(0, res.Content.ReadableBytes);
            Assert.Equal("", res.Content.ToString(Encoding.ASCII));
            Assert.Equal("Netty", res.TrailingHeaders.Get((StringCharSequence)"X-Test"));
        ...
        static void AssertEncodedResponse(EmbeddedChannel ch)
        {
            ...
            Assert.IsNotType<IHttpContent>(res);
            Assert.Equal("chunked", res.Headers.Get(HttpHeaderNames.TransferEncoding).ToString());
            Assert.Null(res.Headers.Get(HttpHeaderNames.ContentLength));
